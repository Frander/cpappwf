...
Future<List<VisitsDetailsStruct>> updateOrAddVisitDetail(
  ...
  debugPrint('🔍 === INICIO DE FUNCIÓN2 updateOrAddVisitDetail ===');
  debugPrint('📥 Parámetros recibidos:');
  debugPrint('   - idActivityStatus: $idActivityStatus');
  debugPrint('   - idStepParent: $idStepParent');
  debugPrint('   - statusOption: "$statusOption"');
  debugPrint('   - statusResponse: "$statusResponse"');
  debugPrint('   - rememberStatus: $rememberStatus');
  debugPrint('   - defaultStatus: "$defaultStatus"');

  // Validar que la lista no sea nula
  visitsList ??= [];

  // LOG: Contenido de la lista actual
  debugPrint('📋 Lista actual tiene ${visitsList.length} elementos:');
  ...
  if (idActivityStatus <= 0) {
    debugPrint('❌ Error: idActivityStatus debe ser mayor a 0');
    return visitsList;
  }
  ...
  try {
    ...
    debugPrint('🔍 Buscando en la lista...');
    int existingIndex = -1;
    ...
    debugPrint('🎯 Resultado de búsqueda: existingIndex = $existingIndex');
    ...
    debugPrint('❌ Error al procesar el registro: $e');
