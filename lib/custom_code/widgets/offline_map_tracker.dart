...
class OfflineMapTracker extends StatefulWidget {
  const OfflineMapTracker({
    super.key,
    this.width,
    this.height,
  ...
  @override
  State<OfflineMapTracker> createState() => _OfflineMapTrackerState();
}

class _OfflineMapTrackerState extends State<OfflineMapTracker>
  ...
  void initState() {
    ...
    // Verificar si hay datos del lote antes de cargar
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      if (widget.idHeadquarter != null) {
        await _validateAndLoadData();
      } else {
        // Si no hay headquarter, solo cargar datos normalmente
        _loadAllData();
      }
  ...
  /// Valida que existan datos del lote en SQLite antes de cargar
  /// Si no existen, muestra diálogo y llama a syncInstallModule
  Future<void> _validateAndLoadData() async {
    try {
      debugPrint(
          '🔍 Verificando datos del lote ${widget.idHeadquarter} en SQLite...');

      final bool hasData = await _checkHeadquarterData(widget.idHeadquarter!);

      if (!hasData) {
        debugPrint('⚠️ No hay datos del lote sincronizados');
        ...
        debugPrint('✅ Datos del lote encontrados en SQLite');
      }
  ...
  Future<void> _syncHeadquarterDataWithLoading() async {
    ...
    try {
      debugPrint(
          '🔄 Iniciando sincronización del lote ${widget.idHeadquarter}...');
  ...
  Future<void> _loadAllData() async {
    try {
      ...
      // Cargar datos del headquarter si existe
      if (widget.idHeadquarter != null) {
        // Cargar Type_points PRIMERO (necesarios para colores)
        await _loadTypePoints(database);

        // Cargar datos completos del headquarter
        await _loadHeadquarter(widget.idHeadquarter!, database);

        // Ejecutar todas las cargas secuencialmente para evitar condiciones de carrera
        await _loadVirtualPoints(widget.idHeadquarter!, database);
        ...
        await _loadPolygon(widget.idHeadquarter!, database);
        await _loadProducts(widget.idHeadquarter!, database);
        await _loadCoordinates(widget.idHeadquarter!, database);
  ...
  Future<void> _loadHeadquarter(int headquarterId, Database database) async {
    try {
      debugPrint('🏢 Cargando datos del headquarter...');
      ...
      if (results.isNotEmpty) {
        ...
        debugPrint('✅ Headquarter cargado: ${_headquarter!.name}');
      } else {
  ...
  Future<void> _loadVirtualPoints(int headquarterId, Database database) async {
    try {
      debugPrint('🟣 Cargando Virtual Points...');
      ...
      debugPrint('✅ Virtual Points cargados: ${_virtualPoints.length}');

      if (mounted) {
        setState(() {});

        // Centrar automáticamente en los puntos virtuales después de cargarlos
  ...
  Future<void> _loadPolygon(int headquarterId, Database database) async {
    try {
      ...
      _polygonPoints = results.map((row) {
      ...
      debugPrint('✅ Polígono cargado: ${_polygonPoints.length} puntos');
  ...
  Future<void> _loadProducts(int headquarterId, Database database) async {
    try {
      ...
      debugPrint(
          '✅ Productos cargados: ${_products.length} (${productResults.length} en total, $productsWithCoordinates con coordenadas)');

      // Log detallado de los primeros productos para debugging
      if (_products.isNotEmpty && _products.length <= 5) {
        ...
        for (var i = 0; i < 3 && i < _products.length; i++) {
  ...
  Future<void> _loadCoordinates(int headquarterId, Database database) async {
    try {
      debugPrint('🔵 Cargando Coordenadas (Polígonos de Exclusión)...');
          ...
          try {
            ...
            if (jsonData is List) {
              ...
              if (polygonPoints.length >= 3) {
                ...
                _coordinates.add(CoordinateData(
      ...
      debugPrint(
          '✅ Coordenadas cargadas: $totalPolygons polígonos con $totalPoints puntos totales');
  ...
  Future<List<VirtualPoint>?> _getOptimizedRouteFromSQLite(
    ...
    try {
      ...
      if (routes.isEmpty) {
        ...
        debugPrint('      - Headquarter: ${widget.idHeadquarter}');
  ...
  Future<void> _generateSimulatedRoute(
    ...
    if (!skipExistingCheck && widget.idHeadquarter != null) {
      final exists = await _routeExists(widget.idHeadquarter!, config);
      if (exists) {
    ...
    if (widget.idHeadquarter != null &&
        config.pattern != RoutePattern.rutaOptimizada) {
      try {
  ...
  Future<void> _showSimulationConfigDialog() async {
    final config = await showDialog<RouteSimulationConfig>(
      ...
      builder: (context) => _SimulationConfigDialog(
        ...
        headquarterId: widget.idHeadquarter,
  ...
  Future<void> _saveExclusionZoneModificationForMultiplePoints({
    ...
    try {
      ...
      if (widget.idHeadquarter != null) {
        ...
        await _loadCoordinates(widget.idHeadquarter!, db);
